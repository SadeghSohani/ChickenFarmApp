...
public class MainActivity extends AppCompatActivity implements View.OnClickListener, Callback<RequestResponse> {
    ...
    private Button btnBuy, btnGetAll, btnInsert;
    private RecyclerView recycler;
    private EditText edtFeedingCompany;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        edtFeedingCompany = findViewById(R.id.edt_feedingCompany);
                ...
                for(int i = 0 ; i < chickensList.size() ; i++) {
                    Call<RequestResponse> call = api.updateChicken(
                            new RequestBody(
                                    ...
                                    chickensList.get(i).getSlaughterHouse(),
                                    "Damghan Jooje"
                            )
                    );
                    call.enqueue(this);
                ...
                for(int i = 0 ; i < chickensList.size() ; i++) {
                    Call<RequestResponse> call = api.updateChicken(
                            new RequestBody(
                                    ...
                                    edtFeedingCompany.getText().toString().trim(),
                                    chickensList.get(i).getMotherCompany(),
                                    chickensList.get(i).getSlaughterHouse(),
                                    chickensList.get(i).getOwner()
                            )
                    );
                    call.enqueue(this);
                }
